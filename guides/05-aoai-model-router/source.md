...
4. 결과에 따라 **routing mode**와 **model subset**을 **한 번에 하나씩만** 바꿔가며 조정 (§7.4)
...
Model router는 **LLM이 아니라, 빠른 추론에 최적화된 경량 ML 모델**입니다. 수십만 건의 다양한 시나리오(QA, 코드 생성, 수학적 추론, 요약, 대화, 에이전트 워크플로)로 학습되었습니다.

요청이 들어오면 3단계로 처리합니다.

1. **프롬프트 이해** — **시스템 메시지, 사용자 메시지, 도구 정의, 대화 기록**을 포함한 요청 전체를 분석해 무엇을 요구하는지와 난이도를 판단
2. **모델 선택** — 풀 안에서 이 프롬프트에 가장 좋은 결과를 낼 모델을 추정하고, 설정된 routing mode를 반영
3. **라우팅과 응답** — 선택된 모델로 전달. **라우팅 오버헤드는 LLM 추론 시간의 극히 일부**입니다

- 프롬프트를 **저장하지 않습니다.**
- 사용자의 **접근 권한과 배포 유형에 맞는 모델만** 후보로 삼고, **데이터 존(data zone) 경계를 준수**합니다.
- 요청 단위로 모델을 고르므로, 같은 애플리케이션 안에서도 요청마다 다른 모델이 응답할 수 있습니다.
- **model subset을 바꾸면 라우터가 남은 모델 기준으로 자동 재보정(recalibrate)** 합니다.
...
모드를 바꾸면 하부 모델로 흐르는 **트래픽 분포 자체가 달라집니다.** Azure Monitor에서 관찰할 수 있습니다 (§7.1).

| 모드 | 분포 경향 |
|---|---|
| Balanced | 프롬프트 복잡도에 따라 **모델 풀 전반에 넓게 분산** |
| Cost | 대부분 **작고 저렴한 모델**로, 꼭 필요할 때만 상위 모델로 에스컬레이션 |
| Quality | **프론티어·고성능 모델**이 대부분의 트래픽 처리 |

> 💡 도입 전에 자기 프롬프트 코퍼스로 분포를 미리 보고 싶다면 [ModelRouter-Distribution](https://github.com/guygregory/ModelRouter-Distribution) 실험 도구를 쓸 수 있습니다.
...
1. **model router 배포 이름**으로 필터 (하부 모델은 별도 배포가 아니므로 별도 배포명이 없습니다)
2. 필요하면 **하부 모델별로 split** — 어떤 모델이 트래픽을 얼마나 가져가는지, 즉 **라우팅 분포**가 보입니다 (§2)

### 7.2 비용

Model router의 비용은 **하부 모델들이 발생시킨 비용의 합**입니다. 라우터 자체의 추가 과금은 없으며, **가격 페이지에 명시된 각 모델의 단가**로 청구됩니다.
...
**첫 배포는 최종 구성이 아니라 시작 구성**으로 다루세요. 프로덕션 트래픽을 넘기기 전에 기존 모델 배포를 baseline으로 두고 **응답 품질 · 예상 비용 · 지연**을 비교합니다.

조정은 **hill-climbing** 방식으로 합니다 — baseline 측정 → **레버 하나만 변경** → 재측정 → 개선됐을 때만 유지, 아니면 롤백 → 다음 레버. Model router의 레버는 두 개입니다.

| 레버 | 조정 예시 |
|---|---|
| **Routing mode** | `Balanced` → `Cost` (비용 ↓, 품질 소폭 ↓) |
| **Model subset** | 고가 모델 제외 · 큰 컨텍스트 모델만 남기기 |

> ⚠️ **두 레버를 동시에 바꾸지 마세요.** 비용이 줄어도 원인이 모드인지 서브셋인지 알 수 없어, 다음 반복에서 판단 근거가 사라집니다. 두 레버를 **함께 사용하는 것 자체는 정상**이며, 문제는 "한 번에 같이 바꾸는 것"입니다.

**공정한 비교를 위한 원칙**

- **의미 있는 baseline** — 지금 워크로드를 처리 중인 모델, 또는 대안으로 채택할 직접 배포와 비교
- **트래픽 믹스 반영** — 일반 요청 · 어려운 엣지 케이스 · 긴 입력 · 고영향 작업을 모두 포함하고 **카테고리별로 묶어** 집계가 특정 카테고리의 퇴행을 가리지 않게 할 것
- **애플리케이션 설정 고정** — 프롬프트·시스템 지시문·출력 한도를 동일하게 유지 (설정 차이가 라우팅 효과처럼 보일 수 있음)
- **사람 검토 계획** — 자동 채점은 추정치입니다. 규제·도메인 특화·판정 불가 사례는 전문가가 확인
- **작거나 편향된 표본 주의** — 중요한 카테고리의 샘플이 적으면 프롬프트를 더 추가

**관찰 → 다음에 시도할 것**

| 관찰 | 고려할 조치 |
|---|---|
| 품질이 요구 수준 미달 | **Quality 모드** 또는 model subset 제한 |
| 품질은 괜찮은데 절감이 적음 | **Cost 모드** 시도 후 고영향 카테고리 재확인 |
| 특정 카테고리만 다르게 동작 | 전체 설정을 바꾸기 전에 **그 카테고리만** 별도 평가 |
| 결과가 불확실 | 대표 프롬프트 추가 · 판정 불가 사례 검토 후 재실행 |
| 사용 불가·미승인 모델에 의존 | 배포 리전에서 사용 가능하고 정책상 허용된 subset 선택 |
| 꼬리 지연(p90/p95)이 요구 초과 | **프로덕션 유사 트래픽·동시성**에서 재검증 |

> 💡 비교를 자동화하려면 [Model Router Auto Evaluation toolkit](https://github.com/microsoft-foundry/Model-Router-Auto-Evaluation)을 쓸 수 있습니다. 실행할 때마다 **baseline · 라우터 버전 · routing mode · model subset · 프롬프트 데이터셋 · 판정 기준**을 함께 기록해야 나중에 결과를 재현하고 원인을 귀속시킬 수 있습니다.

통제된 비교만으로는 프로덕션 조건을 다 재현하지 못합니다. 전면 도입 전에 **제한적 프로덕션 테스트**로 품질 신호 · 실제 비용 · 중앙값과 꼬리 지연 · 오류와 페일오버 동작 · 선택 모델 분포를 관찰하세요. **결정적 모델 선택이 필요한 요청은 직접 모델 배포로 남겨두는 하이브리드**도 정당한 결론입니다.

선택한 구성은 끝이 아니라 **새 baseline**입니다. 트래픽 믹스 · 라우팅 설정 · 지원 모델 · 애플리케이션 동작 · 가격이 바뀌면 평가를 반복하세요.
...
| 항목 | 내용 | 대응 |
|---|---|---|
| **컨텍스트 윈도우** | **하부 모델 중 가장 작은 모델 기준**으로 제한됨. 더 큰 컨텍스트는 "운 좋게 큰 모델로 라우팅될 때만" 성공 | model subset으로 큰 컨텍스트 모델만 선택 · 프롬프트 요약/절단 · 임베딩 검색(RAG)으로 축소 |
| **입력 모달리티** | **이미지 입력은 지원**되지만 **라우팅 판단은 텍스트만 기준**. **오디오 입력은 처리하지 않음** | 오디오는 직접 모델 배포 사용 |
| **파라미터 호환성** | 추론 모델 선택 시 `temperature` 등 무시 | 결정적 출력이 필요하면 subset에서 추론 모델 제외 |
| **응답 모델 비고정** | 요청마다 다른 모델이 응답 | 감사 요건이 있으면 `model` 필드를 로깅. 결정적 선택이 필수면 직접 배포 유지 |
| **라우터 오버헤드** | 라우팅 결정 자체에도 시간이 듦 (추론 시간 대비 극히 일부) | 지연 민감 워크로드는 **Cost 모드**로 작은 모델 비중을 높이는 편이 유리 |
...
| **쿼터** | RPM/TPM 한도가 **구독 사용 티어(Tier 1~6)** 에 따라 스케일됨. Global Standard가 Data Zone Standard보다 넉넉함 | [Quota tiers](https://learn.microsoft.com/azure/foundry/openai/quotas-limits) 확인 후 상향 요청 · 지수 백오프 재시도 |

### 8.1 트러블슈팅 빠른 참조

| 증상 | 원인 | 조치 |
|---|---|---|
| 배포 실패 | 리소스가 미지원 리전에 있음 | 지원 리전 확인 (§3.3) |
| Claude로 라우팅되지 않음 | Claude 미배포 | 모델 카탈로그에서 먼저 배포 |
| Context exceeded 오류 | 최소 컨텍스트 모델 기준 초과 | 프롬프트 축소 또는 큰 컨텍스트 모델로 subset 구성 |
| 예상과 다른 모델 선택 | routing mode / subset 설정 | 모드와 subset 재검토 |
| 지연이 높음 | 라우터 오버헤드 + 하부 모델 처리 시간 | Cost 모드로 작은 모델 비중 ↑ |
| Rate limit 초과 | 라우터 배포의 TPM 한도 도달 | 쿼터 상향 또는 지수 백오프 재시도 |
...
- ☐ 오디오 입력이 필요한 경로는 **직접 모델 배포**로 분리했다
...
- 하부 모델 컨텍스트 윈도우 비교: https://learn.microsoft.com/azure/foundry/foundry-models/concepts/models-sold-directly-by-azure
