...
| 단계 | 조치 | 해결하는 문제 | 적용 모델 |
|---|---|---|---|
| **1** | **앞 1,024 토큰을 문자 단위로 동일하게 유지** | 프롬프트 불안정으로 인한 구조적 miss | **전 모델** |
| **2** | **캐시 보존 기간 연장** (최대 24시간) | 유휴 구간에서 캐시가 만료되는 문제 | 확장 캐싱 지원 모델 (§4.1) |
| **3** | **`prefix` + `prompt_cache_key` 조합의 농도 조절** (~15 RPM 임계) | 라우팅 분산·과밀로 인한 miss | **GPT-5.6 이상만** |

> ⚠️ **3단계는 GPT-5.6 이상 전용입니다.** `prompt_cache_key`는 문서상 GPT-5.6 이상 모델 계열에만 정의되어 있습니다. 그 이전 모델을 쓴다면 3단계를 건너뛰고 **§4 → §6(배포 유형) → §7(APIM)** 순으로 진행하세요.
...
| 기능 | GPT-4o ~ 5.5 | GPT-5.6+ |
|---|---|---|
| 프롬프트 캐싱 (자동) | ✅ | ✅ |
| `prompt_cache_retention` (`24h`) | ✅ (지원 모델 한정, §4.1) | ❌ deprecated |
| `prompt_cache_key` | ❌ 문서화되지 않음 | ✅ |
| `prompt_cache_options` | ❌ → **`400` 오류** | ✅ |
| `prompt_cache_breakpoint` | ❌ → **`400` 오류** | ✅ (PTU-M 제외) |
...
- [ ] *(5.6+)* `prompt_cache_key`가 **안정적 해시**로 샤딩되어 있다 (§5.3)
- [ ] *(5.6+)* 키당 트래픽이 **~15 RPM 이하**다 (§5.2)
...
> ⚠️ **적용 범위 — GPT-5.6 이상 모델 계열 전용**
>
> `prompt_cache_key`는 문서상 **GPT-5.6 이상**에만 정의되어 있습니다. 그 이전 모델(GPT-4o ~ 5.5)을 쓴다면 이 장을 건너뛰고 **§4 보존 기간 연장** → **§6 배포 유형** → **§7 APIM** 순으로 진행하세요.
>
> 문서가 명시한 `400` 오류 대상은 `prompt_cache_options` / `prompt_cache_breakpoint` 두 가지이고 `prompt_cache_key`는 그 목록에 없습니다. 따라서 이전 모델에 보내도 오류 없이 **무시될 가능성**은 있지만, **효과가 있다는 보장은 없습니다.** 검증된 동작이 아니므로 의존하지 마세요.
...
flowchart TB
    ...
    B -->|"아니오"| M{"GPT-5.6 이상인가?"}
    M -->|"아니오"| E["§6 Regional / PTU 전환"]
    M -->|"예"| C{"키당 트래픽이<br/>15 RPM을 넘는가?"}
    C -->|"예"| C1["§5 키 샤딩으로 분산"]
...
| `prompt_cache_key`가 효과 없음 | **GPT-5.6 미만 모델인가** (미지원) | §5 도입부 |
